...
const QuizFilterBar = ({ handleFilteredQuizzes }: QuizFilterBarProps) => {
  ...
  const handleSearchTermChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    debouncedSetSearchTerm(e.target.value); // Update debounced value
  };
  ...
  const fetchFilteredQuizzes = async () => {
    ...
    const response = await axios.get('/cs-quizzes/search', {
      ...
      params: {
        ...
        page: page,
        size: 9,
        keyword: debouncedSearchTerm, // Use debounced search term
      },
    });
    return response.data.data;
  };
  ...
  const { data } = useQuery({
    queryKey: ['filteredQuizzes', category, difficulty, sortOrder, page, debouncedSearchTerm],
    queryFn: fetchFilteredQuizzes,
      ...
      <div className="flex justify-between border-gray-600">
        ...
        <div className="flex justify-center items-center gap-4">
          <input
            ...
            placeholder="검색어를 입력하세요..."
            value={searchTerm}
            onChange={handleSearchTermChange} // Use debounced change handler
          />
